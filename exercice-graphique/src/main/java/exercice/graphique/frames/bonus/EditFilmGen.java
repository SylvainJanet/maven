...
public class EditFilmGen extends JDialog {

	private static final long serialVersionUID = 4232891356091931658L;
	...
	public static void main(String[] args) {
		try {
			EditFilmGen dialog = new EditFilmGen(null, new Film());
	 ...
	 * Create the dialog.
	 */
	public EditFilmGen(Dialog f, Film filmToEdit) {
		super(f);
		setTitle("Modification d'un produit - BONUS générique");
		...
		textField.setText(filmToEdit.getTitle());
		...
		chckbxNewCheckBox.setSelected(filmToEdit.isWatched());
...

		{
			...
			getContentPane().add(buttonPane, BorderLayout.SOUTH);
			{
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						filmToEdit.setTitle(textField.getText());
						filmToEdit.setWatched(chckbxNewCheckBox.isSelected());
						if (chckbxNewCheckBox.isSelected()) {
							filmToEdit.setRating(slider.getValue());
						} else {
							filmToEdit.setRating(null);
						}
						filmService.update(filmToEdit);
						try {
							((BonusFilmGen) EditFilmGen.this.getOwner()).refreshFilmList();
						} catch (Exception ex) {
							ex.printStackTrace();
						}
						EditFilmGen.this.dispose();
					}
				});
